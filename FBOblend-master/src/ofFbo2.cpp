...
#ifdef TARGET_OPENGLES
bool ofFbo2::bglFunctionsInitialized=false;
...
bool ofFbo2::Settings::operator!=(const Settings & other){
...
static void releaseFB(GLuint id){
    if(getIdsFB().find(id)!=getIdsFB().end()){
        ...
        ofLogWarning("ofFbo2") << "releaseFB(): something's wrong here, releasing unknown frame buffer id " << id;
        glDeleteFramebuffers(1, &id);
    }
}
...
static void releaseRB(GLuint id){
    if(getIdsRB().find(id)!=getIdsRB().end()){
        ...
        ofLogWarning("ofFbo2") << "releaseRB(): something's wrong here, releasing unknown render buffer id " << id;
        glDeleteRenderbuffers(1, &id);
    }
}
...
int	ofFbo2::_maxColorAttachments = -1;
int	ofFbo2::_maxDrawBuffers = -1;
int	ofFbo2::_maxSamples = -1;
...
ofFbo2::ofFbo2(const ofFbo2 & mom){
...
ofFbo2 & ofFbo2::operator=(const ofFbo2 & mom){
...
bool ofFbo2::checkGLSupport() {
#ifndef TARGET_OPENGLES
    
    if (!ofIsGLProgrammableRenderer()){
        if(ofGLCheckExtension("GL_EXT_framebuffer_object")){
            ofLogVerbose("ofFbo2") << "GL frame buffer object supported";
        }else{
            ofLogError("ofFbo2") << "GL frame buffer object not supported by this graphics card";
            return false;
        }
    }
    ...
    ofLogVerbose("ofFbo2") << "checkGLSupport(): "
    ...
    ofLogVerbose("ofFbo2") << extensions;
#endif
...
void ofFbo2::allocate(int width, int height, int internalformat, int numSamples) {
...
void ofFbo2::allocate(Settings _settings) {
    ...
    if(_settings.numSamples > maxSamples() && maxSamples() > -1) {
        ofLogWarning("ofFbo2") << "allocate(): clamping numSamples " << _settings.numSamples << " to maxSamples " << maxSamples() << " for frame buffer object" << fbo;
        _settings.numSamples = maxSamples();
    }
    
    if(_settings.depthStencilAsTexture && _settings.numSamples){
        ofLogWarning("ofFbo2") << "allocate(): multisampling not supported with depthStencilAsTexture, setting 0 samples for frame buffer object " << fbo;
    ...
    if( _settings.depthStencilAsTexture ){
        ...
        ofLogWarning("ofFbo2") << "allocate(): depthStencilAsTexture is not available for iOS";
    }
#endif
    ...
    fboTextures = fbo;
    if(_settings.numSamples){
        ofLogWarning("ofFbo2") << "allocate(): multisampling not supported in OpenGL ES";
    }
#endif
    ...
    if(_settings.colorFormats.size() > 0) {
        ...
        ofLogWarning("ofFbo2") << "allocate(): no color buffers specified for frame buffer object " << fbo;
    }
    ...
    /* UNCOMMENT OUTSIDE OF DOING RELEASES
     ...
     if(settings != _settings) ofLogWarning("ofFbo2") << "allocation not complete, passed settings not equal to created ones, this is an internal OF bug";
     
     */
}

bool ofFbo2::isAllocated(){
    return bIsAllocated;
}

GLuint ofFbo2::createAndAttachRenderbuffer(GLenum internalFormat, GLenum attachmentPoint) {
    GLuint buffer;
...
void ofFbo2::createAndAttachTexture(GLenum internalFormat, GLenum attachmentPoint) {
    ofTexture tex;
...
void ofFbo2::attachTexture(ofTexture & tex, GLenum internalFormat, GLenum attachmentPoint) {
...
void ofFbo2::createAndAttachDepthStencilTexture(GLenum target, GLint internalformat, GLenum  attachment, GLenum transferFormat, GLenum transferType){
...
void ofFbo2::createAndAttachDepthStencilTexture(GLenum target, GLint internalformat, GLenum  attachment){
...
void ofFbo2::begin(bool setupScreen) {
    if(!bIsAllocated) return;
    ofPushView();
    if(ofGetGLRenderer()){
        ofGetGLRenderer()->setCurrentFBO((ofFbo*)this);  //TODO This could cause huge bugs and problems... fix this
    }
    ofViewport();
    if(setupScreen){
...
void ofFbo2::setActiveDrawBuffer(int i){
    if(!bIsAllocated) return;
#ifndef TARGET_OPENGLES
    if (i < getNumTextures()){
        ...
        ofLogWarning("ofFbo2") << "setActiveDrawBuffer(): fbo " << fbo << " couldn't set texture " << i << ", only " << getNumTextures() << "allocated";
    }
#endif
}

void ofFbo2::setActiveDrawBuffers(const vector<int>& ids){
    if(!bIsAllocated) return;
#ifndef TARGET_OPENGLES
    ...
    for(int i=0; i < (int)ids.size(); i++){
        ...
        if (id < getNumTextures()){
            ...
            ofLogWarning("ofFbo2") << "setActiveDrawBuffers(): fbo " << fbo << " couldn't set texture " << i << ", only " << getNumTextures() << "allocated";
        }
    }
...
void ofFbo2::activateAllDrawBuffers(){
    if(!bIsAllocated) return;
#ifndef TARGET_OPENGLES
    vector<GLenum> attachments;
    for(int i=0; i < getNumTextures(); i++){
        if (i < getNumTextures()){
            ...
            ofLogWarning("ofFbo2") << "activateAllDrawBuffers(): fbo " << fbo << " couldn't set texture " << i << ", only " << getNumTextures() << "allocated";
        }
    }
    glDrawBuffers(attachments.size(),&attachments[0]);
#endif
...
void ofFbo2::setDefaultTextureIndex(int defaultTexture)
{
    defaultTextureIndex = defaultTexture;
}

int ofFbo2::getDefaultTextureIndex()
{
    return defaultTextureIndex;
}

ofTexture& ofFbo2::getTextureReference(){
    return getTextureReference(defaultTextureIndex);
}

ofTexture& ofFbo2::getTextureReference(int attachmentPoint) {
    updateTexture(attachmentPoint);
    
    return textures[attachmentPoint];
}

void ofFbo2::setAnchorPercent(float xPct, float yPct){
    getTextureReference().setAnchorPercent(xPct, yPct);
}

void ofFbo2::setAnchorPoint(float x, float y){
    getTextureReference().setAnchorPoint(x, y);
}

void ofFbo2::resetAnchor(){
    getTextureReference().resetAnchor();
}

void ofFbo2::readToPixels(ofPixels & pixels, int attachmentPoint){
    if(!bIsAllocated) return;
#ifndef TARGET_OPENGLES
...
void ofFbo2::readToPixels(ofShortPixels & pixels, int attachmentPoint){
    if(!bIsAllocated) return;
#ifndef TARGET_OPENGLES
    getTextureReference(attachmentPoint).readToPixels(pixels);
#else
...
void ofFbo2::readToPixels(ofFloatPixels & pixels, int attachmentPoint){
    if(!bIsAllocated) return;
#ifndef TARGET_OPENGLES
    getTextureReference(attachmentPoint).readToPixels(pixels);
#else
...
void ofFbo2::updateTexture(int attachmentPoint) {
    if(!bIsAllocated) return;
...
void ofFbo2::draw(float x, float y) {
    draw(x, y, settings.width, settings.height);
}


void ofFbo2::draw(float x, float y, float width, float height) {
    if(!bIsAllocated) return;
    getTextureReference().draw(x, y, width, height);
}


GLuint ofFbo2::getFbo() {
    return fbo;
}

float ofFbo2::getWidth() {
    return settings.width;
}


float ofFbo2::getHeight() {
    return settings.height;
}


bool ofFbo2::checkStatus() {
    GLenum status = glCheckFramebufferStatus(GL_FRAMEBUFFER);
    switch(status) {
        case GL_FRAMEBUFFER_COMPLETE:
            ofLogVerbose("ofFbo2") << "FRAMEBUFFER_COMPLETE - OK";
            return true;
        case GL_FRAMEBUFFER_INCOMPLETE_ATTACHMENT:
            ofLogError("ofFbo2") << "FRAMEBUFFER_INCOMPLETE_ATTACHMENT";
            break;
        case GL_FRAMEBUFFER_INCOMPLETE_MISSING_ATTACHMENT:
            ofLogError("ofFbo2") << "FRAMEBUFFER_INCOMPLETE_MISSING_ATTACHMENT";
            break;
        case GL_FRAMEBUFFER_INCOMPLETE_DIMENSIONS:
            ofLogError("ofFbo2") << "FRAMEBUFFER_INCOMPLETE_DIMENSIONS";
            break;
        case GL_FRAMEBUFFER_INCOMPLETE_FORMATS:
            ofLogError("ofFbo2") << "FRAMEBUFFER_INCOMPLETE_FORMATS";
            break;
        case GL_FRAMEBUFFER_UNSUPPORTED:
            ofLogError("ofFbo2") << "FRAMEBUFFER_UNSUPPORTED";
            break;
#ifndef TARGET_OPENGLES
        case GL_FRAMEBUFFER_INCOMPLETE_DRAW_BUFFER:
            ofLogWarning("ofFbo2") << "FRAMEBUFFER_INCOMPLETE_DRAW_BUFFER";
            break;
        case GL_FRAMEBUFFER_INCOMPLETE_READ_BUFFER:
            ofLogError("ofFbo2") << "FRAMEBUFFER_INCOMPLETE_READ_BUFFER";
            break;
        case GL_FRAMEBUFFER_INCOMPLETE_MULTISAMPLE:
            ofLogError("ofFbo2") << "GL_FRAMEBUFFER_INCOMPLETE_MULTISAMPLE";
            break;
#endif
        default:
            ofLogError("ofFbo2") << "UNKNOWN ERROR " << status;
            break;
...
ofTexture & ofFbo2::getDepthTexture(){
    if(!settings.depthStencilAsTexture){
        ofLogError("ofFbo2") << "getDepthTexture(): frame buffer object " << fbo << " not allocated with depthStencilAsTexture";
    }
